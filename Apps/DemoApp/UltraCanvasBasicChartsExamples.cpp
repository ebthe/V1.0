...
static struct ChartControls {
    ...
    std::shared_ptr<UltraCanvasUIElement> UltraCanvasDemoApplication::CreateJitterPlotChartsExamples() {
        auto jitterData = std::make_shared<ChartDataVector>();
        std::random_device rd;
        std::mt19937 gen(rd());
        std::uniform_real_distribution<> yDist(10, 100);
        std::uniform_int_distribution<> categoryDist(1, 5);

        std::vector<ChartDataPoint> jitterPoints;
        std::vector<std::string> categories = {"A", "B", "C", "D", "E"};

        for (int i = 0; i < 50; ++i) {
            int cat = categoryDist(gen);
            double value = yDist(gen);
            jitterPoints.emplace_back(cat, value, 0, categories[cat - 1], value);
        }
        jitterData->LoadFromArray(jitterPoints);

        auto container = std::make_shared<UltraCanvasContainer>("JitterPlotExamples", 100, 0, 0, 800, 700);

        auto descLabel = std::make_shared<UltraCanvasLabel>("JitterPlotDescription", 2005, 50, 20, 700, 60);
        descLabel->SetText("Jitter Plot Example - Scatter plot with random horizontal jitter.\n"
                          "Useful for visualizing categorical data with many overlapping points.\n"
                          "Features: Customizable jitter, multiple shapes, tooltips, zoom, and pan.");
        descLabel->SetFontSize(12);
        descLabel->SetTextColor(Color(50, 50, 50, 255));
        container->AddChild(descLabel);

        std::shared_ptr<UltraCanvasJitterPlotElement> jitterPlot =
                CreateJitterPlotElement("jitterPlot", 1005, 50, 100, 600, 400);

        jitterPlot->SetDataSource(jitterData);
        jitterPlot->SetChartTitle("Categorical Distribution with Jitter");
        jitterPlot->SetPointColor(Color(255, 99, 132, 255));
        jitterPlot->SetPointSize(8.0f);
        jitterPlot->SetPointShape(UltraCanvasJitterPlotElement::PointShape::Circle);
        jitterPlot->SetJitterAmount(0.4f);
        jitterPlot->SetEnableTooltips(true);
        jitterPlot->SetEnableZoom(true);
        jitterPlot->SetEnablePan(true);

        container->AddChild(jitterPlot);
        ...
        auto btnCycleShapes = std::make_shared<UltraCanvasButton>("btnCycleJitterShapes", 3041,
                                                                  buttonX, buttonY, buttonWidth, buttonHeight);
        btnCycleShapes->SetText("Cycle Shapes");

        std::vector<UltraCanvasJitterPlotElement::PointShape> jitterShapes = {
                UltraCanvasJitterPlotElement::PointShape::Circle,
                UltraCanvasJitterPlotElement::PointShape::Square,
                UltraCanvasJitterPlotElement::PointShape::Triangle,
                UltraCanvasJitterPlotElement::PointShape::Diamond
        };

        static int currentJitterShape = 0;
        btnCycleShapes->SetOnClick([jitterPlot, jitterShapes]() {
            currentJitterShape = (currentJitterShape + 1) % jitterShapes.size();
            jitterPlot->SetPointShape(jitterShapes[currentJitterShape]);
        });
        container->AddChild(btnCycleShapes);

        buttonX += buttonWidth + buttonSpacing;
        auto btnToggleJitter = std::make_shared<UltraCanvasButton>("btnToggleJitter", 3042,
                                                                    buttonX, buttonY, buttonWidth, buttonHeight);
        btnToggleJitter->SetText("Toggle Jitter");
        static bool jitterEnabled = true;
        btnToggleJitter->SetOnClick([jitterPlot]() {
            jitterEnabled = !jitterEnabled;
            jitterPlot->SetEnableJitter(jitterEnabled);
        });
        container->AddChild(btnToggleJitter);

        buttonX += buttonWidth + buttonSpacing;
        auto btnRandomize = std::make_shared<UltraCanvasButton>("btnRandomizeJitter", 3043,
                                                                 buttonX, buttonY, buttonWidth, buttonHeight);
        btnRandomize->SetText("Randomize Data");
        btnRandomize->SetOnClick([jitterPlot]() {
            ...
            std::vector<ChartDataPoint> newPoints;
            std::vector<std::string> categories = {"A", "B", "C", "D", "E"};

            for (int i = 0; i < 50; ++i) {
                int cat = categoryDist(gen);
                double value = yDist(gen);
                newPoints.emplace_back(cat, value, 0, categories[cat - 1], value);
            }
            newData->LoadFromArray(newPoints);
            jitterPlot->SetDataSource(newData);
        });
        container->AddChild(btnRandomize);
